...
file = open("picosat_960_results/{file_name}.tmp.{num}", "w")
sys.stdout = file
'''.format(file_name=name,num=i)
       
                    file = open("picosat_960_results/{file_name}.tmp.{num}".format(file_name=name, num=i), "w")
                    t0 = time.time()
                    sp = subprocess.Popen(["picosat-960/picosat", "{file_name}".format(file_name=file_name)], stdout=file)
                    sp.communicate()
                    t1 = time.time()
                    avg += (t1-t0)
                    file.close()
                ...
                for i in range(0, loop):
                    path = "picosat_960_results/{file_name}.tmp.{num}".format(file_name=name, num=i)
    ...
    with open("picosat_960_results/{tests}".format(tests=tests), "w") as o:
        o.write(results)
