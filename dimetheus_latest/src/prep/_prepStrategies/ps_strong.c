...
void prep_strategy_strong_printHelp(){
	printf("c      %-3d: STRONG:\n", PREP_STRATEGY_STRONG);
    printf("c           Behavior: Strong preprocessing, ELS, NHBR, UNH, SE, STR, PLE, DL-FLD, NIVER.\n");
}

void prep_strategy_strong_reset(){
	#ifdef VERBOSE_PREP
	printf("c   PREP: Strategy reset (STRONG)...\n");
	#endif
	pre_resetModule();
}

void prep_strategy_strong_init(){
	#ifdef VERBOSE_PREP
	printf("c   PREP: Strategy init (STRONG)...\n");
	#endif
	pre_initModule();
...
void prep_strategy_strong_dispose(){
	#ifdef VERBOSE_PREP
	printf("c   PREP: Strategy dispose (STRONG)...\n");
	#endif
	pre_disposeModule();
}

void prep_strategy_strong_cycleA_init(){
	int32_t i;
...
void prep_strategy_strong_cycleA_exec(){
...
void prep_strategy_strong_cycleB_init(){
	int32_t i;
	uint32_t random, numToPermute;
	variable *v;
	literal *lPos, *lNeg;
...
void prep_strategy_strong_cycleB_exec(){
	int32_t oldNumC = 0, oldNumV = 0, changes = 0;
	clause *c;
	...
	while (changes || (oldNumC != f.m_eo_used + f.m_el_used) || (oldNumV != f.n_vars_e_used)){
		...
		if (cQ_leaf > 1 || cS_leaf > 1 || lS_leaf > 1) {
			++changes;
			continue;
		}
...
void prep_strategy_strong_cycleC_init(){
	//We assume all the queues and stacks are flushed (empty), but we force it just to make sure.
	cQ_flush();
	cS_flush();
	lQ_flush();
...
void prep_strategy_strong_cycleC_exec(){
...
void prep_strategy_strong_execute(){
	#ifdef VERBOSE_PREP
	printf("c   PREP: Strategy execute (STRONG)...\n");
	#endif

	#ifdef VERBOSE_PREP
	printf("c   PREP: Cycle A.\n");
	#endif
	if (prep_returnCode == PREP_UNKNOWN) prep_strategy_strong_cycleA_init();
	if (prep_returnCode == PREP_UNKNOWN) prep_strategy_strong_cycleA_exec();
	#ifdef VERBOSE_PREP
	...
	#endif
	if (prep_returnCode == PREP_UNKNOWN) prep_strategy_strong_cycleB_init();
	if (prep_returnCode == PREP_UNKNOWN) prep_strategy_strong_cycleB_exec();
	#ifdef VERBOSE_PREP
	...
	#endif
	if (prep_returnCode == PREP_UNKNOWN) prep_strategy_strong_cycleC_init();
	if (prep_returnCode == PREP_UNKNOWN) prep_strategy_strong_cycleC_exec();
	#ifdef VERBOSE_PREP
