...
void mp_updateRule_L1I_SIGMAEMSPG_printHelp(){
	printf("c      %-3d: L1I-SIGMAEMSPG:\n", MP_UPDATERULE_L1I_SIGMAEMSPG);
    printf("c           Behavior: Performs ISI-based SIGMAEMSPG with sigma in [0,1]. Sigma=0 gives SP and Sigma=1 gives EMSPG.\n");
}

void mp_updateRule_L1I_SIGMAEMSPG_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L1I-SIGMAEMSPG]:\n", MP_UPDATERULE_L1I_SIGMAEMSPG);
	printf("c     MP:       sigma: %f\n", param_mpSigma);
...
void mp_updateRule_L1I_SIGMAEMSPG_msgUpdate(){
	//This is the level 1 heuristics implementation of sigmaEMSPG with sigma in [0,1].
	...
	for (i = 0; i < perm_lfsr_perLen; ++i){
		...
		while (*litC != 0){
			...
			denominator =  (U*(ONE-S) + S + param_mpSigma*((s+u+ONE)*U*S + u + s - U - S));
			...
			if (denominator > ZERO){
				...
				*deltas = (U*(ONE-S) + param_mpSigma*(u*U+(s-U)*(ONE-S))) / denominator;
				#ifdef COLLECTSTATS
				++stats_mp_computation_numDiv;
				stats_mp_computation_numMult += 8;
				#endif
			} else {
				...
				*deltas = HALF;
				#ifdef COLLECTSTATS
				stats_mp_computation_numMult += 4;
				#endif
			}
...
void mp_updateRule_L1I_SIGMAEMSPG_computeBias(){
	float_ty muPlus, muMinus, muPM, mu, F, T, sf, st;
	mpVariable* var;
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Check the paper on the computation of magnetization and biases in sigmaEMSPG.
		...
		if (sf > 0 || st > 0){
			...
			muPM    = T*F + param_mpSigma*(T*F*(sf+st-ONE));					//Compute \mu^+-
			muPlus  = T*(ONE-F) + param_mpSigma*(T*(st+F-ONE) + sf*(ONE-F));	//Compute \mu^+
			muMinus = F*(ONE-T) + param_mpSigma*(F*(sf+T-ONE) + st*(ONE-T));	//Compute \mu^-
