...
void mp_updateRule_L0_BP_printHelp(){
	printf("c      %-3d: L0-BP:\n", MP_UPDATERULE_L0_BP);
    printf("c           Behavior: Performs Belief Propagation.\n");
}

void mp_updateRule_L0_BP_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L0-BP]:\n", MP_UPDATERULE_L0_BP);
...
void mp_updateRule_L0_BP_msgUpdate(){
	//This is the level 0 heuristics implementation of BP.
...
void mp_updateRule_L0_BP_computeBias(){
	float_ty F,T,muPlus,muMinus,mu;
	mpVariable* var;
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Check the paper on the computation of magnetization and biases in BP.
