...
void mp_updateRule_L1I_SIGMAEMBPG_printHelp(){
	printf("c      %-3d: L1I-SIGMAEMBPG:\n", MP_UPDATERULE_L1I_SIGMAEMBPG);
    printf("c           Behavior: Performs ISI-based SIGMAEMBPG with sigma in [0,1]. Sigma=0 gives BP and Sigma=1 gives EMBPG.\n");
}

void mp_updateRule_L1I_SIGMAEMBPG_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L1I-SIGMAEMBPG]:\n", MP_UPDATERULE_L1I_SIGMAEMBPG);
	printf("c     MP:       sigma: %f\n", param_mpSigma);
...
void mp_updateRule_L1I_SIGMAEMBPG_msgUpdate(){
	//This is the level 1 heuristics implementation of sigmaEMBPG with sigma in [0,1].
	...
	for (i = 0; i < perm_lfsr_perLen; ++i){
		...
		while (*litC != 0){
			...
			denominator = (U + S + param_mpSigma*((u-ONE)*U + (s-ONE)*S + u + s));
			...
			if (denominator > ZERO){
				...
				*deltas = (U + param_mpSigma*((u-ONE)*U + s)) / denominator;
				#ifdef COLLECTSTATS
				++stats_mp_computation_numDiv;
				stats_mp_computation_numMult += 5;
				#endif
			} else {
				...
				*deltas = HALF;
				#ifdef COLLECTSTATS
				stats_mp_computation_numMult += 3;
				#endif
			}
...
void mp_updateRule_L1I_SIGMAEMBPG_computeBias(){
	float_ty muPlus, muMinus, F, T, sf, st, mu;
	mpVariable* var;
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Check the paper on the computation of magnetization and biases in sigmaEMBPG.
		...
		if (sf > 0 || st > 0){
			...
			muPlus  = T + param_mpSigma*((st-ONE)*T + sf);	//Compute \mu^+
			muMinus = F + param_mpSigma*((sf-ONE)*F + st);	//Compute \mu^-

			mu      = muPlus + muMinus;						//Compute \mu
