...
void mp_updateRule_L1I_RHOEMSPG_printHelp(){
	printf("c      %-3d: L1I-RHOEMSPG:\n", MP_UPDATERULE_L1I_RHOEMSPG);
    printf("c           Behavior: Performs ISI-based RHOEMSPG with rho in [0,1]. Rho=0 gives EMBPG and Rho=1 gives EMSPG.\n");
}

void mp_updateRule_L1I_RHOEMSPG_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L1I-RHOEMSPG]:\n", MP_UPDATERULE_L1I_RHOEMSPG);
	printf("c     MP:       rho:   %f\n", param_mpRho);
...
void mp_updateRule_L1I_RHOEMSPG_msgUpdate(){
	//This is the level 1 heuristics implementation of rhoEMSPG with rho in [0,1].
	...
	for (i = 0; i < perm_lfsr_perLen; ++i){
		...
		while (*litC != 0){
			...
			denominator = (u*(ONE+U*(ONE-param_mpRho*(ONE-S))) + s*(ONE+S*(ONE-param_mpRho*(ONE-U))));
			...
			if (denominator > ZERO){
				...
				*deltas = (u*U + s*(ONE-param_mpRho*S)) / denominator;
				#ifdef COLLECTSTATS
				++stats_mp_computation_numDiv;
				stats_mp_computation_numMult += 9;
				#endif
			} else {
				...
				*deltas = HALF;
				#ifdef COLLECTSTATS
				stats_mp_computation_numMult += 6;
				#endif
			}
...
void mp_updateRule_L1I_RHOEMSPG_computeBias(){
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Check the paper on the computation of magnetization and biases in rhoEMSPG.
		...
		if (sf > ZERO || st > ZERO){
			...
			muPM  = param_mpRho*(st + sf)*T*F;		    //Compute \mu^+-
			muPlus  = st*T + sf*(ONE - param_mpRho*F);	//Compute \mu^+
			muMinus = sf*F + st*(ONE - param_mpRho*T);	//Compute \mu^-
