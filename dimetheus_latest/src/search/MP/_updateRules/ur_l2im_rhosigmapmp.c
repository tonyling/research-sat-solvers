...
void mp_updateRule_L2IM_RHOSIGMAPMP_printHelp(){
	printf("c      %-3d: L2IM-RHOSIGMAPMP:\n", MP_UPDATERULE_L2IM_RHOSIGMAPMP);
    printf("c           Behavior: Performs ISI-based RHOSIGMAPMP with force-field and rho, sigma in [0,1], Phi in [-1.0,1.0].\n");
}

void mp_updateRule_L2IM_RHOSIGMAPMP_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L2IM-RHOSIGMAPMP]:\n", MP_UPDATERULE_L2IM_RHOSIGMAPMP);
	printf("c     MP:       rho:   %f\n", param_mpRho);
	printf("c     MP:       sigma: %f\n", param_mpSigma);
	printf("c     MP:       Phi: %f\n", param_mpPhi);
...
void mp_updateRule_L2IM_RHOSIGMAPMP_msgUpdate(){
	...
	const float_ty phiA = 1-fabs(param_mpPhi);	//Just a linear reduction on the original message (penalty).
	...
	for (i = 0; i < perm_lfsr_perLen; ++i){
		...
		while (*litC != 0){
			...
			//It now is newOmega = (1-|Phi|) \omega(c,v) + (|Phi| + sign(lit) * Phi)/2
...
void mp_updateRule_L2IM_RHOSIGMAPMP_computeBias(){
