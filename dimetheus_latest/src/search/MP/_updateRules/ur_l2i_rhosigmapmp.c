...
void mp_updateRule_L2I_RHOSIGMAPMP_printHelp(){
	printf("c      %-3d: L2I-RHOSIGMAPMP:\n", MP_UPDATERULE_L2I_RHOSIGMAPMP);
    printf("c           Behavior: Performs ISI-based RHOSIGMAPMP with rho and sigma in [0,1].\n");
}

void mp_updateRule_L2I_RHOSIGMAPMP_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L2I-RHOSIGMAPMP]:\n", MP_UPDATERULE_L2I_RHOSIGMAPMP);
	printf("c     MP:       rho:   %f\n", param_mpRho);
	printf("c     MP:       sigma: %f\n", param_mpSigma);
...
void mp_updateRule_L2I_RHOSIGMAPMP_msgUpdate(){
	//This is the level 2 heuristics implementation of rhosigmaPMP with rho,sigma in [0,1].
...
void mp_updateRule_L2I_RHOSIGMAPMP_computeBias(){
