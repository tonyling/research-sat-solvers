...
void mp_updateRule_L0_EMSPG_printHelp(){
	printf("c      %-3d: EMSPG:\n", MP_UPDATERULE_L0_EMSPG);
    printf("c           Behavior: Performs Expectation Maximization SP Global (EMSPG).\n");
}

void mp_updateRule_L0_EMSPG_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: EMSPG]:\n", MP_UPDATERULE_L0_EMSPG);
...
void mp_updateRule_L0_EMSPG_msgUpdate(){
	//This is the level 0 heuristics implementation of EMSPG.
	...
	for (i = 0; i < perm_lfsr_perLen; ++i){
		...
		while (*litC != 0){
			...
			if (denominator > ZERO){
				...
				*deltas = (u*U + s*(ONE-S)) / denominator;
				#ifdef COLLECTSTATS
				++stats_mp_computation_numDiv;
...
void mp_updateRule_L0_EMSPG_computeBias(){
	float_ty muPlus, muMinus, muPM, mu, F, T, st, sf;
	mpVariable* var;
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Check the paper on the computation of magnetization and biases in EMSPG.
		...
		if (st > ZERO || sf > ZERO){
			...
			muPM    = (st + sf)*T*F;			//Compute \mu^+-
			muPlus  = st*T + sf*(ONE - F);		//Compute \mu^+
			muMinus = sf*F + st*(ONE - T);		//Compute \mu^-
