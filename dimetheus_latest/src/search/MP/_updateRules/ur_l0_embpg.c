...
void mp_updateRule_L0_EMBPG_printHelp(){
	printf("c      %-3d: L0-EMBPG:\n", MP_UPDATERULE_L0_EMBPG);
    printf("c           Behavior: Performs Expectation Maximization BP Global (EMBPG).\n");
}

void mp_updateRule_L0_EMBPG_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L0-EMBPG]:\n", MP_UPDATERULE_L0_EMBPG);
...
void mp_updateRule_L0_EMBPG_msgUpdate(){
	//This is the level 0 heuristics implementation of EMBPG.
...
void mp_updateRule_L0_EMBPG_computeBias(){
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Grab the variables freedom to be assigned positively and negatively: T and F, and the clause counts st and sf.
		//Check the paper on the computation of magnetization and biases in EMBPG.
		...
		if (sf > 0 || st > 0){
			...
			muPlus  = st*T + sf;			//Compute \mu^+
			muMinus = sf*F + st;			//Compute \mu^-
