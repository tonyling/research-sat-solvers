...
void mp_updateRule_L1I_RHOSP_printHelp(){
	printf("c      %-3d: L1I-RHOSP:\n", MP_UPDATERULE_L1I_RHOSP);
    printf("c           Behavior: Performs ISI-based RHOSP with rho in [0,1]. Rho=0 gives BP and Rho=1 gives SP.\n");
}

void mp_updateRule_L1I_RHOSP_prepare(){
	//In this method, we see to it that the mp_messageFactors array has sufficient size.
	#ifdef VERBOSE_MP
	printf("c     MP:     UR-Plug-in [%-3d: L1I-RHOSP]:\n", MP_UPDATERULE_L1I_RHOSP);
	printf("c     MP:       rho:   %f\n", param_mpRho);
...
void mp_updateRule_L1I_RHOSP_msgUpdate(){
	//This is the level 1 heuristics implementation of rhoSP with rho in [0,1].
	...
	for (i = 0; i < perm_lfsr_perLen; ++i){
		...
		while (*litC != 0){
			...
			denominator = U * (ONE - param_mpRho*S) + S;
			...
			if (denominator > ZERO){
				...
				*deltas = (U * (ONE - param_mpRho*S)) / denominator;
				#ifdef COLLECTSTATS
				++stats_mp_computation_numDiv;
		...
		//What follows is the update of all the omegas send by the clause, and an update of all the T/F values.
		litC = GET_MP_CLS_LITS(c);
		omegasC = GET_MP_CLS_OMEGAS(c);
		deltas = mp_clauseDeltas;
		while (*litC != 0){
			...
			//We update the omega(c,v) message of this clause to the current variable.
			*omegasC = newOmega;
...
void mp_updateRule_L1I_RHOSP_computeBias(){
	...
	for (i = 0; i < f.n_vars_e_used; ++i){
		...
		//Check the paper on the computation of magnetization and biases in rhoSP.
		...
		if (F > ZERO || T > ZERO){
			...
			muPM 	= param_mpRho*F*T;			//Compute \mu^+-
			muPlus 	= T*(ONE-param_mpRho*F);	//Compute \mu^+
			muMinus = F*(ONE-param_mpRho*T);	//Compute \mu^-
