...
void sls_pickAndFlipVarRule_probsatPB_printHelp(){
	printf("c      %-3d: PROBSAT-PB:\n", SLS_PICKANDFLIPVARRULE_PROBSATPB);
    printf("c           Behavior: Using ProbSAT polynomial (brake only) probability scores to pick variables to flip.\n");
}

void sls_pickAndFlipVarRule_probsatPB_prepare(){
	#ifdef VERBOSE_SLS
	printf("c     SLS:     PAFVR-Plug-in [%-3d: PROBSAT-PB]:\n", SLS_PICKANDFLIPVARRULE_PROBSATPB);
	printf("c     SLS:       Maximum literal occurrence encountered: %d.\n", sls_litMaxOcc);
	printf("c     SLS:       -slsProbsatCB %f.\n", param_slsProbsatCB);
	printf("c     SLS:       -slsProbsatCBShift %f.\n", param_slsProbsatCBShift);
	#endif
	int32_t numOcc;
	...
	for (numOcc = 1; numOcc < sls_litMaxOcc+1; ++numOcc){
		sls_probsat_scoreLookupB[numOcc] = (float_ty) pow(param_slsProbsatCBShift + numOcc, -param_slsProbsatCB);
	}
...
void sls_pickAndFlipVarRule_probsatPB_paf(){
	//Performs a try using a ProbSAT polynomial probability distribution using only brake score.
	uint64_t numFlips = 0;
	slsClause **occs;
	slsClause *c;
	...
	do {
		...
		do {
			//Given the brake, we remove the raw score for the literal (not normalized) from the total mass.
			++litC;
			totalScore -= sls_probsat_scoreLookupB[GET_SLS_VAR_BRAKE(*litC)];
		...
		while ((c = *occs) != NULL){
			//We will increase the numTrue, but before that, we will handle the clause.
