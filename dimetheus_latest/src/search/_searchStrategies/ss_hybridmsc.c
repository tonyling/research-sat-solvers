...
void search_strategy_hybridmsc_printHelp(){
	printf("c      %-3d: HYBRIDMSC\n", SEARCH_STRATEGY_HYBRIDMSC);
    printf("c           Behavior: Performs MP, SLS, and CDCL in a clever combination[MP, SLS, CDCL].\n");
}

void search_strategy_hybridmsc_reset(){
	#ifdef VERBOSE_SEARCH
	printf("c   SEARCH: Strategy reset (HYBRIDMSC)...\n");
	#endif
	inp_resetPhase();
	...
	search_strategy_hybridmsc_midBlockSize = 1;
	search_strategy_hybridmsc_assignmentMax = 1;
	search_strategy_hybridmsc_assignmentMin = 0;
	search_strategy_hybridmsc_maxMPImpact = ZERO;
	search_strategy_hybridmsc_maxMagForSLS = ZERO;
}

void search_strategy_hybridmsc_init(){
	#ifdef VERBOSE_SEARCH
	printf("c   SEARCH: Strategy init (HYBRIDMSC)...\n");
	#endif
	...
	search_strategy_hybridmsc_midBlockSize = param_searchMIDBlockSize * f.n_vars_e_used;
	if (search_strategy_hybridmsc_midBlockSize == 0U) ++search_strategy_hybridmsc_midBlockSize;
	search_strategy_hybridmsc_assignmentMax = search_strategy_hybridmsc_midBlockSize;
	search_strategy_hybridmsc_assignmentMin = 0;
	search_strategy_hybridmsc_maxMPImpact = f.n_vars_e_used * param_searchImpactMaxFactor;
	search_strategy_hybridmsc_maxMagForSLS = param_searchMaxMagForSLS;
	...
	printf("c   SEARCH:   param_searchRetreatFact                  = %f\n", param_searchRetreatFact);
	printf("c   SEACRH:   param_searchMaxMagForSLS                 = %f\n", param_searchMaxMagForSLS);
	printf("c   SEARCH:   search_strategy_hybridmsc_midBlockSize   = %u\n", search_strategy_hybridmsc_midBlockSize);
	printf("c   SEARCH:   search_strategy_hybridmsc_assignmentMax  = %u\n", search_strategy_hybridmsc_assignmentMax);
	printf("c   SEARCH:   search_strategy_hybridmsc_assignmentMin  = %u\n", search_strategy_hybridmsc_assignmentMin);
	printf("c   SEARCH:   search_strategy_hybridmsc_maxMPImpact    = %f\n", search_strategy_hybridmsc_maxMPImpact);
	printf("c   SEARCH:   search_strategy_hybridmsc_maxMagForSLS   = %f\n", search_strategy_hybridmsc_maxMagForSLS);
	#endif
}

void search_strategy_hybridmsc_dispose(){
	#ifdef VERBOSE_SEARCH
	printf("c   SEARCH: Strategy dispose (HYBRIDMSC)...\n");
	#endif
	inp_disposePhase();
	mp_disposeModule();
	sls_disposeModule();
...
void search_strategy_hybridmsc_execute(){
	//This search strategy will use MP as a guide to detect strongly biased variables. Then, we call either CDCL to advance
	//the search of SLS, if probing seems to be fruitful.
	#ifdef VERBOSE_SEARCH
	printf("c   SEARCH: Strategy execute (HYBRIDMSC)...\n");
	#endif
	...
	do {
		...
		if (restart == 1){
			...
			search_strategy_hybridmsc_maxMPImpact = f.n_vars_e_used * param_searchImpactMaxFactor;
			#ifdef VERBOSE_SEARCH
			printf("c   SEARCH: Restarting. VARS: %6d CLS: %6d MINASSIGN: %6d MAXASSIGN: %6d MAXIMPACT: %f\n",
					f.n_vars_e_used, f.m_eo_used + f.m_el_used,	search_strategy_hybridmsc_assignmentMin,
					search_strategy_hybridmsc_assignmentMax, search_strategy_hybridmsc_maxMPImpact);
			#endif
			...
			if (mp_returnCode == MP_ERROR){
				printf("c ERROR. HYBRIDMSC asked for MP but this resulted in an error (A). HYBRIDMSC failed.\n");
				...
				//We first determine if it is time to call for the SLS solver on a sub-formula that looks easy.
				if (mp_globalMagnetization < search_strategy_hybridmsc_maxMagForSLS){
					//MP succeeded in computing new biases and the formula has a small magnetization. We call for the SLS.
					#ifdef VERBOSE_SEARCH
					printf("c   SEARCH: Low magnetization %f. Calling for SLS.\n", mp_globalMagnetization);
					#endif
					search_strategy_hybridmsc_assumeAndLocalSearch();
				}
			}

			//In case the SLS did not find a satisfying assignment, we continue putting the workload on the CDCL in order to
			//further reduce the formula -- continuing MID.
			if (sls_returnCode == SLS_ERROR){
				printf("c ERROR. HYBRIDMSC asked for SLS but this resulted in an error (A). HYBRIDMSC failed.\n");
				search_returnCode = SEARCH_ERROR;break;
			...
			//not too big (this would indicate that MP is not helpful anyway). If it is too big, we give the SLS a chance and
			//if that fails, we just increase the CDCL assignment target.
			if (retainedImpact < search_strategy_hybridmsc_maxMPImpact){
				//It seems that doing MP gives good guidance as the impact of learned things is low.
				search_strategy_hybridmsc_assumeAndUpdate(); if (search_returnCode != SEARCH_UNKNOWN){break;}
				...
				//further reduce the formula -- continuing MID.
				if (sls_returnCode == SLS_ERROR){
					printf("c ERROR. HYBRIDMSC asked for SLS but this resulted in an error (B). HYBRIDMSC failed.\n");
					search_returnCode = SEARCH_ERROR;break;
				} else if (sls_returnCode == SLS_UNKNOWN){
					if (mp_returnCode == MP_CONVERGED_PARAMAG){
						...
						search_strategy_hybridmsc_assignmentMax = f.n_initial +2;
						#ifdef VERBOSE_SEARCH
						printf("c   SEARCH: Low  retained impact. PARAMAGNETIC. VARS: %6d CLS: %6d MAXASSIGN: %6d\n",
								f.n_vars_e_used, f.m_eo_used + f.m_el_used,	search_strategy_hybridmsc_assignmentMax);
						#endif
					} else {
						...
						search_strategy_hybridmsc_assignmentMin = param_searchRetreatFact*((float_ty)CDCL_DECSTACK_GET_NUM_ASSIGNMENTS());
						search_strategy_hybridmsc_assignmentMax = CDCL_DECSTACK_GET_NUM_ASSIGNMENTS() + search_strategy_hybridmsc_midBlockSize;
						if (search_strategy_hybridmsc_assignmentMax >= f.n_vars_e_used){
							//Just to make sure that the CDCL cannot get stuck.
							search_strategy_hybridmsc_assignmentMax = f.n_initial +2;
						}
						#ifdef VERBOSE_SEARCH
						if (mp_returnCode == MP_UNCONVERGED){
							printf("c   SEARCH: Low retained impact. UNCONVERGED. VARS: %6d CLS: %6d MINASSIGN: %d MAXASSIGN: %6d\n",
									f.n_vars_e_used, f.m_eo_used + f.m_el_used,	search_strategy_hybridmsc_assignmentMin,
									search_strategy_hybridmsc_assignmentMax);
						} else {
							printf("c   SEARCH: Low retained impact. Non-paramagnetic(%f). VARS: %6d CLS: %6d MINASSIGN: %d MAXASSIGN: %6d\n",
									mp_globalMagnetization,	f.n_vars_e_used, f.m_eo_used + f.m_el_used,
									search_strategy_hybridmsc_assignmentMin,
									search_strategy_hybridmsc_assignmentMax);
						}
						#endif
				...
				//It seems that doing MP gives bad guidance as the impact of learned things is high. We update the biases
				//anyway and now call for the SLS. After that we reset the retained impact.
				search_strategy_hybridmsc_assumeAndUpdate(); if (search_returnCode != SEARCH_UNKNOWN){break;}
				search_strategy_hybridmsc_assignmentMin = param_searchRetreatFact*((float_ty)CDCL_DECSTACK_GET_NUM_ASSIGNMENTS());
				search_strategy_hybridmsc_assignmentMax = CDCL_DECSTACK_GET_NUM_ASSIGNMENTS() + search_strategy_hybridmsc_midBlockSize;

				#ifdef VERBOSE_SEARCH
				printf("c   SEARCH: HIGH RETAINED IMPACT. VARS: %6d CLS: %6d MINASSIGN: %6d MAXASSIGN: %6d\n",
						f.n_vars_e_used, f.m_eo_used + f.m_el_used,	search_strategy_hybridmsc_assignmentMin,
						search_strategy_hybridmsc_assignmentMax);
				printf("c   SEARCH: Resetting retained impact and calling SLS.\n");
				#endif
				retainedImpact = ZERO;
				//search_strategy_hybridmsc_assumeAndLocalSearch();
				if (sls_returnCode == SLS_ERROR){
					printf("c ERROR. HYBRIDMSC asked for SLS but this resulted in an error (C). HYBRIDMSC failed.\n");
					search_returnCode = SEARCH_ERROR;break;
				}
			}
			//In case the SLS was not called or did not return a satisfying assignment, we force the CDCL to continue.
			if (sls_returnCode == SLS_UNKNOWN){
				cdcl_extern_learn(0);
			} else {
				#ifdef VERBOSE_SEARCH
				printf("c   SEARCH: SLS found a satisfying assignment.\n");
				#endif
			}
		}

		//Check the result of the computations.
		if (sls_returnCode == SLS_SAT){
			#ifdef VERBOSE_SEARCH
			printf("c   SEARCH: SLS found a satisfying assignment. HYBRIDMSC succeeded.\n");
			#endif
			search_intern_transferSLSAss();
			search_returnCode = SEARCH_SAT;
		} else if (cdcl_returnCode == CDCL_SAT){
			#ifdef VERBOSE_SEARCH
			printf("c   SEARCH: CDCL found a satisfying assignment. HYBRIDMSC succeeded.\n");
			#endif
			search_intern_transferCDCLAss();
			search_returnCode = SEARCH_SAT;
		} else if (cdcl_returnCode == CDCL_UNSAT){
			#ifdef VERBOSE_SEARCH
			printf("c   SEARCH: CDCL found a global conflict. HYBRIDMSC succeeded.\n");
			#endif
			...
			if (CDCL_DECSTACK_GET_NUM_ASSIGNMENTS_IN_LEVEL_ZERO() > 0){
				...
				impact += search_strategy_hybridmsc_retainNewEquivalences();	if (search_returnCode != SEARCH_UNKNOWN) break;
				//We retain highly impacting clauses that have been learned by the CDCL.
				impact += search_strategy_hybridmsc_convertImpactingClauses();	if (search_returnCode != SEARCH_UNKNOWN) break;
				//We retain unit clauses that have been found.
				impact += search_strategy_hybridmsc_retainNewUnits();			if (search_returnCode != SEARCH_UNKNOWN) break;
				...
				if (inp_returnCode == INP_UNSAT){
					#ifdef VERBOSE_SEARCH
					printf("c   SEARCH: INP returned unsatisfiable. HYBRIDMSC succeeded.\n");
					#endif
					search_returnCode = SEARCH_UNSAT;
				} else if (inp_returnCode == INP_SAT){
					#ifdef VERBOSE_SEARCH
					printf("c   SEARCH: INP returned satisfiable. HYBRIDMSC succeeded.\n");
					#endif
					search_returnCode = SEARCH_SAT;
				} else if (inp_returnCode == INP_ERROR){
					#ifdef VERBOSE_SEARCH
					printf("c   SEARCH: INP returned an error. HYBRIDMSC failed.\n");
					#endif
			...
			#ifdef VERBOSE_SEARCH
			printf("c   SEARCH:   CDCL returned an unexpected return code. HYBRIDMSC failed.\n");
			#endif
			search_returnCode = SEARCH_ERROR;
		}
...
uint32_t search_strategy_hybridmsc_isCDCLClauseSubsumed(cdclClause *cc){
...
float_ty search_strategy_hybridmsc_updateMessages(clause *c){
...
float_ty search_strategy_hybridmsc_retainNewEquivalences(){
	...
	for (i=0; i < f.n_vars_e_used; ++i){
		...
		if (Y != 0){
			...
			if (j == GET_LIT_NUM_NUMIMP_USED(X)){
				...
				clsImpact = search_strategy_hybridmsc_updateMessages(f.clauses_el[f.m_el_used-1]);
...
float_ty search_strategy_hybridmsc_convertImpactingClauses(){
	...
	for (i = 0; i < CDCL_GET_NUM_LEARNED_CLAUSES(); ++i){
		...
		if (GET_CDCL_CLS_SIZE(cc) == 2){
			...
			if (!IS_CDCL_CLS_REASON(cc)
					&& (zeros || impact < param_searchMinClsImpact || search_strategy_hybridmsc_isCDCLClauseSubsumed(cc))) {
				continue;
			}
...
float_ty search_strategy_hybridmsc_retainNewUnits(){
	...
	while (i < CDCL_DECSTACK_GET_NUM_ASSIGNMENTS_IN_LEVEL_ZERO()){
		...
		if (!IS_VAR_UNASSIGNED(v)) {
			printf("c ERROR. HYBRIDMSC tried to transfer units and found a variable that is already assigned.\n");
			search_returnCode = SEARCH_ERROR;
			return ZERO;
		}
...
void search_strategy_hybridmsc_assumeAndUpdate(){
	...
	for (dl = 1; dl < CDCL_GET_DECLEVEL(); ++dl){
		...
		if ((c = main_complexup_propagate(0)) != NULL){
			printf("c ERROR. HYBRIDMSC tried to follow the CDCL decisions and ran into a conflict (UM). HYBRIDMSC failed.\n");
			search_returnCode = SEARCH_ERROR;
			return;
		}
	}
	...
	mp_extern_iterate(1);
	//If MP did converge, we can compute biases. Otherwise, we assume the data is useless and stick to the old biases.
	if (mp_returnCode == MP_ERROR){
		printf("c ERROR. HYBRIDMSC asked for MP but this resulted in an error. HYBRIDMSC failed.\n");
...
void search_strategy_hybridmsc_assumeAndLocalSearch(){
	//This method will first assign all the variables according to the CDCL assignment. It will then perform a run of local
	//search in order to find a satisfying assignment under the given CDCL assignments as assumptions. After that, we
	//undo all temporary assignments on the main decision stack.
	variable *v;
	uint32_t dl;
	int32_t decID;
	...
	for (dl = 1; dl < CDCL_GET_DECLEVEL(); ++dl){
		...
		MAIN_COMPLEXUP_ADD_DEC(decID, v);
		if (main_complexup_propagate(0) != NULL){
			printf("c ERROR. HYBRIDMSC tried to follow the CDCL decisions and ran into a conflict (US). HYBRIDMSC failed.\n");
			search_returnCode = SEARCH_ERROR;
			return;
		}
	}

	//We then do the local search on the sub-formula.
