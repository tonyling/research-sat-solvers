...
void cdcl_inprocessingRule_simple_printHelp(){
	printf("c      %-3d: SIMPLE:\n", CDCL_INPROCESSINGRULE_SIMPLE);
    printf("c           Behavior: Reduces the formula and does TER UNH TRL ELS and SE.\n");
}
...
void cdcl_inprocessingRule_simple_prepare(){
	...
	#ifdef VERBOSE_CDCL
	printf("c     CDCL:     IR-Plug-in [%-3d: SIMPLE]:\n", CDCL_INPROCESSINGRULE_SIMPLE);
...
cdclClause* cdcl_inprocessingRule_simple_inprocess(){
	...
	printf("c     CDCL:   In-Processing [SIMPLE]: Starting to in-process (%d conflicts distance to last in-processing)...\n",
			cdcl_numConflicts-cdcl_inprocessingRules_lastConflict);
	printf("c     CDCL:   In-Processing [SIMPLE]:   Remaining variables: %d\n", initNumV);
	printf("c     CDCL:   In-Processing [SIMPLE]:   Remaining clauses:   %d\n", initNumC);
	printf("c     CDCL:   In-Processing [SIMPLE]:   ");fflush(stdout);
	#endif
	...
	#ifdef VERBOSE_CDCL
	printf("\n");
	printf("c     CDCL:   In-Processing [SIMPLE]:   Removed variables: %d (%d remaining)\n",
			initNumV - cdcl_vars_e_used, cdcl_vars_e_used);
	printf("c     CDCL:   In-Processing [SIMPLE]:   Removed clauses: %d (%d remaining)\n",
