...
void cdcl_maintenanceRule_lbdinact_printHelp(){
	printf("c      %-3d: LBDINACT:\n", CDCL_MAINTENANCERULE_LBDINACT);
    printf("c           Behavior: Dynamically switches between LBD and INACTIVITY based on standard deviation of LBD.\n");
}

void cdcl_maintenanceRule_lbdinact_prepare(){
	//The LBDINACT based CDBM preparation.
	...
	#ifdef VERBOSE_CDCL
	printf("c     CDCL:     MR-Plug-in [%-3d: LBDINACT]:\n", CDCL_MAINTENANCERULE_LBDINACT);
...
void cdcl_maintenanceRule_lbdinact_incAct(cdclClause* c){
...
void cdcl_maintenanceRule_lbdinact_maintain(){
	//The LBDINACT rule removes half of the learned clauses.
	#ifdef COLLECTSTATS
	...
	if (numCls == ZERO){
		//If no clause can be removed, we do not do any maintenance. We just increase the target.
		++cdcl_maintenanceRules_numPerformed;
