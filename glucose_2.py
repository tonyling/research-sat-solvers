...
file = open("glucose_2_results/{file_name}.tmp.{num}", "w")
sys.stdout = file
'''.format(file_name=name,num=i)
       
                    file = open("glucose_2_results/{file_name}.tmp.{num}".format(file_name=name, num=i), "w")
                    t0 = time.time()
                    sp = subprocess.Popen(["glucose_2.0/glucose_static", "{file_name}".format(file_name=file_name)], stdout=file)
                    sp.communicate()
                    t1 = time.time()
                    avg += (t1-t0)
                    file.close()
                ...
                for i in range(0, loop):
                    path = "glucose_2_results/{file_name}.tmp.{num}".format(file_name=name, num=i)
    ...
    with open("glucose_2_results/{tests}".format(tests=tests), "w") as o:
        o.write(results)
